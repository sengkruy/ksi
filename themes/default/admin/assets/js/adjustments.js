...
function loadItems() {
    ...
    if (localStorage.getItem('qaitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, oqty = item.row.oqty, item_qty = item.row.qty, item_option = item.row.option, item_code = item.row.code, item_costs = item.row.cost,item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            tr_html += '<td>' + USDollar.format(item_costs)+'</td>';
