$(document).ready(function () {
    ...
    check_add_item_val();
    if (site.settings.set_focus != 1 && 0) {
        $('#add_item').focus();
    }
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id,
                ...
                item_p_cost = formatDecimal(item.row.pcost);
            ...
            console.log(item_p_cost);
            var red_bg='';
            if(item_cost!=item_p_cost)
            {
                console.log("not eq");
                red_bg = 'style="background-color:yellow;"';
            }
            tr_html +=
                '<td ' +red_bg+ ' class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' +
                row_no +
                '" value="' +
                item_cost +
...
function add_purchase_item(item) {
    ...
    audio.src = site.url+'assets/uploads/beep.mp3' 
