            ...
            <div class="col-lg-12">
                ...
                <div class="well well-sm no-print">
                    ...
                    <div class="controls table-controls">
                        ...
                        <div class="form-group">
                            <?= lang('style', 'style'); ?>
                            <?php $opts = ['' => lang('select') . ' ' . lang('style'), 99 => 'Label for Shelf',40 => lang('40_per_sheet'), 30 => lang('30_per_sheet'), 24 => lang('24_per_sheet'), 20 => lang('20_per_sheet'), 18 => lang('18_per_sheet'), 14 => lang('14_per_sheet'), 12 => lang('12_per_sheet'), 10 => lang('10_per_sheet'), 50 => lang('continuous_feed')]; ?>
                    ...
                    <?php
                        if ($this->input->post('print')) {
                            if (!empty($barcodes)) {
                                ...
                                $c = 1;
                                if ($style == 12 || $style == 18 || $style == 24 || $style == 40 || $style == 99) {
                                ...
                                foreach ($barcodes as $item) {
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if ($style == 99)
                                        {
                                            ...
                                            echo number_format(ceil(($item['price']*4100)/100)*100) . ' ៛';
                                            echo '</span> ';
                                            echo '</div>';
                                            ...
                                            echo '<span class="barcode_image"><img src="' . admin_url('products/barcode/' . $item['barcode'] . '/' . $item['bcs'] . '/' . $item['bcis']) . '" alt="' . $item['barcode'] . '" class="bcimg" /></span>';
                                            if ($item['price']) {
                                                echo '<span style="line-height:20px;" class="barcode_price">'  . ' ';
                                                if ($item['currencies']) {
                                                    foreach ($currencies as $currency) {
                                                        echo $currency->code . ' ' . $this->sma->formatMoney($item['rprice'] * $currency->rate, 'none') ;
                                                    }
                                                } else {
                                                    echo number_format(ceil(($item['price']*4100)/100)*100) . ' ៛<br>';
                                                    echo '$ ' . $item['price'];
                                                }
                                                echo '</span> ';
                                            }
