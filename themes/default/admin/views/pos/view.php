    ...
    <div id="wrapper">
        <div id="receiptData">
            <div class="no-print">
                ...
                <h1 style="font-weight:bold; color:red;"><?= $inv->reference_no;?></h1>
            ...
            <div id="receipt-data">
                ...
                echo lang('sale_ref') . ': ' . '<b style="font-weight:bold; color:red;">' . $inv->reference_no . '</b><br>';
                    ...
                    <tfoot>
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr>
                                ...
                                <th class="text-right" style="font-size: 30px;"><?=$this->sma->formatMoney($return_sale ? (($inv->grand_total + $inv->rounding) + $return_sale->grand_total) : ($inv->grand_total + $inv->rounding)); ?></th>
                            </tr>
                            <tr>
                                <th>Khmer Riel (1$=<?=$ex_rate;?>)</th>
                                <th class="text-right" style="font-size: 18px;"><?=
substr($this->sma->formatMoney($return_sale ? ((ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)-ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)%1000)+(ceil((ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)%1000)/100))*100) : ((ceil($inv->grand_total*$ex_rate)-ceil($inv->grand_total*$ex_rate)%1000)+(ceil((ceil($inv->grand_total*$ex_rate)%1000)/100))*100)),0,-3); ?></th>
                            </tr>
                            <?php
                        } else {
                            ?>
                            <tr>
                                <th><?=lang('grand_total'); ?></th>
                                <th class="text-right" style="font-size: 30px;"><?=$this->sma->formatMoney($return_sale ? ($inv->grand_total + $return_sale->grand_total) : $inv->grand_total); ?></th>
                            </tr>
                            <tr>
                                <th>Khmer Riel (1$=<?=$ex_rate;?>)</th>
                                <th class="text-right" style="font-size: 18px;"><?=
substr($this->sma->formatMoney($return_sale ? ((ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)-ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)%1000)+(ceil((ceil(($inv->grand_total + $return_sale->grand_total)*$ex_rate)%1000)/100))*100) : ((ceil($inv->grand_total*$ex_rate)-ceil($inv->grand_total*$ex_rate)%1000)+(ceil((ceil($inv->grand_total*$ex_rate)%1000)/100))*100)),0,-3); ?></th>
    ...
    <?php if ($zxcv): ?>
      ...
      //  console.log("current: " +currentTime);
        ...
        // Convert the server time ($inv->date) to milliseconds since the Unix epoch
        var invDate = new Date("<?php echo $inv->date; ?>").getTime();
       // console.log("invDate: " +invDate);
        // Calculate the time difference in seconds
        var timeDifference = (currentTime - invDate) / 1000;
       // console.log("timeDifference: " +timeDifference);
        // Check if the time difference is under 20 seconds
        if (timeDifference < 20) {
            console.log('open');
            $.ajax({
            type: "POST",
            url: "http://127.0.0.1:5000/open_drawer",
            data: { secret_word: 'opmycdw' },
            dataType :  "text",
