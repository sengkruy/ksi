                                ...
                                <table id="totalTable"
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right" style="padding: 5px 10px;font-weight:bold;">
                                            <span id="tds" style="font-size: 20px;">0.00</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="text-right" style="padding:5px 10px 5px 10px; font-size: 14px;border-top: 1px solid #666; border-bottom: 1px solid #333; font-weight:bold; background:#333; color:#FFF;" colspan="2">
                                            <span id="gtotal" style="font-size: 20px;">0.00</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="padding: 5px 10px;font-family: hanuman; border-top: 1px solid #666; border-bottom: 1px solid #333; font-weight:bold; background:#333; color:#FFF;" colspan="2">
                                            សរុបលុយខ្មែរ
                                            
                                            <span id="tship"></span>
                                        </td>
                                        <td class="text-right" style="padding:5px 10px 5px 10px; font-size: 14px;border-top: 1px solid #666; border-bottom: 1px solid #333; font-weight:bold; background:#333; color:#FFF;" colspan="2">
                                            <span id="gtotalkhmer" style="font-size: 20px;">0.00</span>
                                        </td>
                                    </tr>
                                </table>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        // $('#gtotal').on('DOMSubtreeModified', function() {
        // // Get the new value of "gtotal" and calculate the updated value for "gtotalkhmer"
        // var gtotalValue = parseFloat($('#gtotal').text());
        // // var gtotalkhmerValue = gtotalValue * 2;
        
        // const price = gtotalValue;
        // const convertedPrice = Math.ceil((price * 4100) / 100) * 100;
        // const formattedPrice = new Intl.NumberFormat().format(convertedPrice);
        // const result = formattedPrice + ' ៛';
        // // Update the "gtotalkhmer" span with the new value
        //  $('#gtotalkhmer').text(result);
        // });
        

                // Select the target node (gtotal span)
        const gtotalNode = document.querySelector('#gtotal');

        // Create a new MutationObserver instance
        const observer2 = new MutationObserver(function() {
            // Get the new value of "gtotal" and calculate the updated value for "gtotalkhmer"
            var gtotalValue = parseFloat(gtotalNode.textContent);

            const price = gtotalValue;
            const convertedPrice = Math.ceil((price * 4120) / 100) * 100;
            const formattedPrice = new Intl.NumberFormat().format(convertedPrice);
            const result = formattedPrice + ' ៛';
        ...
        // Configure and start the observer to monitor changes in the "gtotal" span
        const observerConfig2 = {
            childList: true, // Watch for changes in the direct children of the target node
            subtree: true, // Watch for changes in the entire subtree of the target node
            characterData: true, // Watch for changes in the text content of the target node
        };

        observer2.observe(gtotalNode, observerConfig2);
        // var blinkInterval;
  
        // function updateTdsStyling() {
        //     var tdsValue = $("#tds").text().trim();
            
        //     // Check if tds is not equal to "0.00" or "(0.00) 0.00"
        //     if (tdsValue !== "0.00" && tdsValue !== "(0.00) 0.00") {
        //     // Apply the red color and start blinking
        //     $("#tds").addClass("red");
        //     if (!blinkInterval) {
        //         blinkInterval = setInterval(function() {
        //         $("#tds").toggleClass("red");
        //         }, 500); // Change the blinking interval as desired (in milliseconds)
        //     }
        //     } else {
        //     // Remove the red color and stop blinking
        //     $("#tds").removeClass("red");
        ...
        // // Check for styling whenever the content changes
        // $("#tds").on("DOMSubtreeModified", function() {
        //     updateTdsStyling();
        // });
        ...
        // Check for styling using MutationObserver
        ...
        observer.observe($("#tds")[0], observerConfig);
...
?>
        ...
        function saveSearchTerm(term, status) {
            var data = {
                term: term,
                status: status,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=admin_url('sales/save_search_term');?>', // Update this with your server URL
                data: data,
        ...
        $("#add_item").autocomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    ...
                    //requested                        bootbox.alert('request');
                        response(data);
                    }
                });
            },
            ...
            select: function (event, ui) {
                ...
                if (ui.item.id !== 0) {
                    ...
                    console.log("stat:2");
                    console.log(ui.item.row.code);
                    saveSearchTerm(ui.item.row.code,2);
                    //data=dt,ui.item,2
        ...
        $('select, .select').select2({minimumResultsForSearch: 30});
