...
class Pos extends MY_Controller
{
        ...
        if (true == $this->form_validation->run() && !empty($products) && !empty($data)) {
            if ($suspend) {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    $redirect_to = $this->pos_settings->after_sale_page ? 'pos' : 'pos/view/' . $sale['sale_id'] . "?zxcv=1";
    ...
    public function view($sale_id = null, $modal = null)
    {
        ...
        $this->data['page_title'] = $this->lang->line('invoice');
        $this->data['zxcv'] = null;
        if ($this->input->get('zxcv')) {
            $this->data['zxcv'] = $this->input->get('zxcv');
        }
        print($this->data['zxcv']);
