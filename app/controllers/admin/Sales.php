...
class Sales extends MY_Controller
{
    ...
    public function save_search_term() {
        ...
        if ($this->db->insert('search_term', $data)) {
            echo json_encode(array('message' => 'Data saved successfully. from controler'));
        } else {
            echo json_encode(array('error' => 'Error saving data: ' . $this->db->error()));
        }
    }
    ...
    public function delete($id = null)
    {
        ...
        if(!$this->Owner && !$this->Admin){
            if($inv->date <= date("Y-m-d H:i:s", strtotime("-1 hours")))
            {
                $this->sma->send_json(['error' => 1, 'msg' => "Cannot delete sale created over an hour."]);
            }
        }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products, $attachments)) {
            ...
            if(!$this->Owner && !$this->Admin){
                if($this->data['inv']->date <= date("Y-m-d H:i:s", strtotime("-1 hours")))
                {
                    $this->session->set_flashdata('error', sprintf('This sale is created over one hour, cannot be edited', 2));
                    redirect($_SERVER['HTTP_REFERER']);
                }
            }
	...
	public function getSalesQuick($warehouse_id = null)
    {
       // $this->sma->checkPermissions('index');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, {$this->db->dbprefix('companies')}.phone, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id ,DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, {$this->db->dbprefix('sales')}.customer,{$this->db->dbprefix('companies')}.phone,{$this->db->dbprefix('companies')}.address,  {$this->db->dbprefix('deliveries')}.delivered_by,sale_status,GROUP_CONCAT(CONCAT(FLOOR({$this->db->dbprefix('sale_items')}.quantity),' = ',{$this->db->dbprefix('sale_items')}.product_name) SEPARATOR '<br>') as productname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
                ->join('companies',' sales.customer_id = companies.id','left')
            ...
            $this->datatables
        ->select("{$this->db->dbprefix('sales')}.id as id ,DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, {$this->db->dbprefix('sales')}.customer,{$this->db->dbprefix('companies')}.phone,{$this->db->dbprefix('companies')}.address,  {$this->db->dbprefix('deliveries')}.delivered_by,sale_status,GROUP_CONCAT(CONCAT(FLOOR({$this->db->dbprefix('sale_items')}.quantity),' = ',{$this->db->dbprefix('sale_items')}.product_name) SEPARATOR '<br>') as productname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                // ->select("{$this->db->dbprefix('sales')}.id as id ,DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller,{$this->db->dbprefix('companies')}.phone, {$this->db->dbprefix('sales')}.customer, sale_status,GROUP_CONCAT(CONCAT(FLOOR({$this->db->dbprefix('sale_items')}.quantity),' = ',{$this->db->dbprefix('sale_items')}.product_name) SEPARATOR '<br>') as productname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
        ...
        if($start_date){
           $start_date = $this->sma->fld($start_date);
            $this->datatables->where('sma_sales.date >=', $start_date);
        }
        if($end_date){
           $end_date = $this->sma->fld($end_date);
            $this->datatables->where('sma_sales.date <=', $end_date);
        }
        if ($this->input->get('shop') == 'yes') {
            $this->datatables->where('shop', 1);
        } elseif ($this->input->get('shop') == 'no') {
    ...
    public function sales_construct()
    {
      //  $this->sma->checkPermissions();
        $this->data['error']      = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
       ...
       // $meta = ['page_title' => lang('products_report'), 'bc' => $bc];
        $this->page_construct('sales/salesfilter', $meta, $this->data);
    }
    public function getSalesWidget(){
        $qs = <<<EOT
        SELECT 
        sum(if(payment_status in('partial','due', 'pending') and sale_status ='completed',1,0)  and sma_sales.pos =0) as 'unpaid'
        ,sum(if(delivered_by = 'unknown' and sale_status ='completed',1,0) and sma_sales.pos =0) as 'unknown_delivery'
        ,sum(if(delivered_by is null and sale_status ='completed',1,0) and sma_sales.pos =0) as 'not_packed_delivery'
        ,sum(if(sale_status ='completed',1,0) and sma_sales.pos =0) as 'total_order'
        ,sum(if(sale_status ='returned',1,0) and sma_sales.pos =0) as 'total_return'
        FROM sma_sales
        left join sma_deliveries on sma_sales.id = sma_deliveries.sale_id
      EOT;
      $start_date = $this->input->get('start_date');
      $end_date = $this->input->get('end_date');
    ...
    public function get_duplicate_sale()
    {
        $start_date = date("Y-m-d 00:00", strtotime("-4 days")); // 5days
        $end_date = date("Y-m-d 23:59");
       // $start_date = "2021-07-22 00:00";
      //  $end_date = "2021-07-22 23:59";
        $qs = <<<EOT
            select 
            a.id,a.date,reference_no, a.customer,sale_status,a.customer, b.phone,b.address,payment_status
            from sma_sales a 
            left join sma_companies b on a.customer_id =b.id
            right join 
            (SELECT id,max(reference_no) rn
            FROM (select 
            a.id,reference_no,  b.phone
            ...
            where a.date>="$start_date" and a.date<="$end_date" and a.sale_status ='completed' and b.phone != '067466300'
            group by phone
            having count(1)>1) dup  
            on b.phone = dup.phone
            where a.date>="$start_date" and a.date<="$end_date" and a.sale_status ='completed'
            ) z
            GROUP BY z.phone ) yy
            on a.reference_no = rn
            order by rn
        EOT;
        $query = $this->db->query($qs);
   
   
        echo json_encode(array('aaData'=>$query->result()));
    ...
    public function indexsalesquick($warehouse_id = null)
    {
       // $this->sma->checkPermissions();
        ...
        $this->page_construct('sales/salesquick', $meta, $this->data);
    }
    public function index($warehouse_id = null)
    {
        ...
        if(!isset($_COOKIE['list_sale_starttime']) || !isset($_COOKIE['list_sale_endtime']) ){
            $end = date('t/m/Y').' 23:59'; // Current month
            $start = date('01/m/Y').' 00:00'; // of current month
            setcookie('list_sale_starttime', $start);
            setcookie('list_sale_endtime', $end);
        }
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('sales')]];
        $meta = ['page_title' => lang('sales'), 'bc' => $bc];
        $this->page_construct('sales/salesfilter', $meta, $this->data);
    ...
    public function suggestions($pos = 0)
    {
        ...
        if ($rows) {
            ...
            log_message('debug', 'This is an rrrrrrrrrrrr message.');
            $this->sma->send_json($pr);
        } else {
            log_message('debug', 'This is an kjlkjlkklkjlkjlkjlkj message.');
    ...
    public function view_delivery($id = null)
    {
        ...
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $deli                = $this->sales_model->getDeliveryByID($id);
        $add = explode("<br>",$deli->address)[0] ."</p>" ;
        $tel = "";
        
        if (preg_match('/Tel:(.*?)Email/', explode("<br>",$deli->address)[1], $match) == 1) {
            $tel =  $match[1];
        }
        $other['add'] = $add;
        $other['tel'] = $tel;
        ...
        $this->data['biller']     = $this->site->getCompanyByID($sale->biller_id);
        $this->data['rows']       = $this->sales_model->getAllInvoiceItemsWithDetails($deli->sale_id);
        $gt = $this->sales_model->getGrandTotal($deli->sale_id);
        $other = array_merge($other,array("grand_total"=>number_format($gt,2)));
        $this->data['user']       = $this->site->getUser($deli->created_by);
        $this->data['page_title'] = lang('delivery_order');
        $this->data['other'] = $other;
        $this->load->view($this->theme . 'sales/view_delivery', $this->data);
    }
	public function back_view_delivery($id = null)
    {
        $this->sma->checkPermissions('deliveries');
