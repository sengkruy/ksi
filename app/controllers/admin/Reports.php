...
class Reports extends MY_Controller
{
    ...
    public function getProductsReport($pdf = null, $xls = null)
    {
        ...
        $pp = "( SELECT product_id, p.ate as date, p.created_by as created_by, SUM(CASE WHEN pi.purchase_id IS NOT NULL THEN quantity ELSE 0 END) as purchasedQty, SUM(quantity_balance) as balacneQty, SUM( unit_cost * quantity_balance ) balacneValue, SUM( (CASE WHEN pi.purchase_id IS NOT NULL THEN (pi.subtotal) ELSE 0 END) ) totalPurchase from {$this->db->dbprefix('purchase_items')} pi LEFT JOIN {$this->db->dbprefix('purchases')} p on p.id = pi.purchase_id WHERE pi.status = 'received' ";
    ...
    public function daily_sales_detail()
    {
        
        if(isset($_POST['monthPick']))
        {
            $monthPick_start = $_POST['monthPick'].'-01';
            $monthPick_end = new DateTime($monthPick_start);
            $monthPick_end = $monthPick_end->format('Y-m-t');
            // var_dump($_POST['monthPick']);
            // var_dump($monthPick_start);
            // var_dump($monthPick_end);
        }
        else
        {
            $monthPick_end = date('Y-m-t');
            $monthPick_start = date('Y-m-01');
            // var_dump("no monthpick");
            // var_dump($monthPick_start);
            // var_dump($monthPick_end);
        }
        ...
        sum(if(d=1,qty,0)) as '1',
        sum(if(d=2,qty,0)) as '2',
        sum(if(d=3,qty,0)) as '3',
        sum(if(d=4,qty,0)) as '4',
        sum(if(d=5,qty,0)) as '5',
        sum(if(d=6,qty,0)) as '6',
        sum(if(d=7,qty,0)) as '7',
        sum(if(d=8,qty,0)) as '8',
        sum(if(d=9,qty,0)) as '9',
        sum(if(d=10,qty,0)) as '10',
        sum(if(d=11,qty,0)) as '11',
        sum(if(d=12,qty,0)) as '12',
        sum(if(d=13,qty,0)) as '13',
        sum(if(d=14,qty,0)) as '14',
        sum(if(d=15,qty,0)) as '15',
        sum(if(d=16,qty,0)) as '16',
        sum(if(d=17,qty,0)) as '17',
        sum(if(d=18,qty,0)) as '18',
        sum(if(d=19,qty,0)) as '19',
        sum(if(d=20,qty,0)) as '20',
        sum(if(d=21,qty,0)) as '21',
        sum(if(d=22,qty,0)) as '22',
        sum(if(d=23,qty,0)) as '23',
        sum(if(d=24,qty,0)) as '24',
        sum(if(d=25,qty,0)) as '25',
        sum(if(d=26,qty,0)) as '26',
        sum(if(d=27,qty,0)) as '27',
        sum(if(d=28,qty,0)) as '28',
        sum(if(d=29,qty,0)) as '29',
        sum(if(d=30,qty,0)) as '30',
        sum(if(d=31,qty,0)) as '31'
        
        from
        (
        SELECT product_id,product_name
        , floor(sum(quantity)) as qty
        ,day(b.date) as d
        ,b.date
        
        FROM sma_sale_items a
        
        left join sma_sales b on a.sale_id=b.id
        where   b.sale_status='completed' and b.date between '$monthPick_start' and '$monthPick_end'
        group by a.product_code, day(b.date)
        )as tt
        group by product_id
        order by product_id
      EOT;
        
        //$sql = "select * from {$this->db->dbprefix('products')} where code in ?";
        $query = $this->db->query($sql);
        $this->load->library('table');
       // echo $this->table->generate($query);
   // $this->load->view($this->theme . 'reports/daily_sales_detail');
        $this->data['db_result'] = $query->result_array();
        $this->page_construct('reports/daily_sales_detail', $meta, $this->data);
        // $this->db
