...
class Products extends MY_Controller
{
    ...
    public function add_adjustment($count_id = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
				...
				$pname = $this->products_model->getProductByID($product_id)->code . ' ' . $this->products_model->getProductByID($product_id)->name . ' ';
                if (!$this->Settings->overselling && 'subtraction' == $type && !$count_id) {
                    if ($variant) {
                        if ($op_wh_qty = $this->products_model->getProductWarehouseOptionQty($variant, $warehouse_id)) {
                            if ($op_wh_qty->quantity < $quantity) {
                                $this->session->set_flashdata('error',$pname . lang('warehouse_option_qty_is_less_than_damage'));
                    ...
                    if ($wh_qty = $this->products_model->getProductQuantity($product_id, $warehouse_id)) {
                        if ($wh_qty['quantity'] < $quantity) {
                            $this->session->set_flashdata('error',$pname . lang('warehouse_qty_is_less_than_damage'));
                            redirect($_SERVER['HTTP_REFERER']);
                        }
                    } else {
                        $this->session->set_flashdata('error',$pname . lang('warehouse_qty_is_less_than_damage'));
    ...
    public function test1()
    {
        ...
        SELECT 
            ...
            COALESCE(wp.quantity, 0) AS pawfect_quantity,
            COALESCE(wp1.quantity, 0) AS petland_quantity,
            s.date,
            COALESCE(wp.quantity, 0) + COALESCE(wp1.quantity, 0) AS total_quantity
        FROM 
            pawfect.sma_products p
        LEFT JOIN 
            pawfect.sma_categories c ON p.category_id = c.id
        LEFT JOIN 
            pawfect.sma_warehouses_products wp ON p.id = wp.product_id AND wp.warehouse_id = 1
        LEFT JOIN 
            petland.sma_products p1 ON p1.code = p.code
        LEFT JOIN 
            petland.sma_warehouses_products wp1 ON p1.id = wp1.product_id AND wp1.warehouse_id = 1
        LEFT JOIN (
            SELECT si.product_code, s.date
            FROM pawfect.sma_sale_items si
            JOIN pawfect.sma_sales s ON si.sale_id = s.id
            WHERE 
                s.date >= '".date('Y-m-d 06:00:00')."'
                AND s.date < '".date('Y-m-d 12:00:00')."'
        ) s ON p.code = s.product_code
        WHERE s.date IS NOT NULL
        HAVING total_quantity <= $osq
        ";
       ...
       // send_to_telegram_group("hello");
        $filepath  = $this->exportStockCount3(array_column($result, 'code'));
        sendTelegramDocument($filepath,null,'Link: ' . base_url(""));
    }
    public function test2()
    {
        ...
        // Define the SQL query
        $sql = "
                -- Query for current quantity of the product sold between interval 
        ...
        LEFT JOIN (
            ...
            JOIN pawfect.sma_sales s ON si.sale_id = s.id
            WHERE 
                s.date >= DATE_FORMAT(NOW(), '%Y-%m-%d 06:00:00')
                AND s.date < DATE_FORMAT(NOW(), '%Y-%m-%d 12:00:00')
        ) s ON p.code = s.product_code
        WHERE s.date IS NOT NULL
        AND COALESCE(wp.quantity, 0) <= $osq  -- OSQ (Out of Stock Quantity) = 5
        AND COALESCE(wp1.quantity, 0) >= 2
        AND COALESCE(wp1.quantity, 0) > COALESCE(wp.quantity, 0);
    ...
    {
        $limit = 100;
        $products = $this->products_model->getProductsByCategoryAndQuantityLimit($limit);
        
         // Generate HTML content
         $html_content = "<html><body><h1>Products with quantity less than $limit:</h1><ul>";
         foreach ($products as $product) {
             $html_content .= "<li><strong>Code:</strong> {$product['code']}, <strong>Name:</strong> {$product['name']}</li>";
         }
         $html_content .= "</ul></body></html>";
         // Save HTML content to a file
        $file = 'assets/p.html';
        $file_path = FCPATH . 'assets/p.html';
        echo $file_path;
        
        $directory = FCPATH . 'assets';
        if (is_dir($directory)) {
        ...
        if (file_put_contents($file_path, $html_content) !== false) {
            ...
            // In your controller or view
             $this->load->helper('telegram');
            sendTelegramDocument($file_path,null,'Link: ' . base_url($file));
           // send_to_telegram_group(base_url($file));
        } else {
    ...
    {
        $limit = 100;
        $products = $this->Product_model->getProductsByCategoryAndQuantityLimit($limit);
        
        // Prepare message
        $message = "Products with quantity less than $limit:\n";
        foreach ($products as $product) {
            $message .= "Code: {$product['code']}, Name: {$product['name']}\n";
        }
         // In your controller or view
         $this->load->helper('telegram');
         send_to_telegram_group($message);
    }
    public function exportStockCount2($product_code=null) {
        $wh = 1 ;//only HQ
        ...
        if(!$product_code){
            ...
            // Split the product codes into an array
            $codesArray = explode(" ", trim($productCodes));
        }
        else{
        ...
        $sheet->setCellValue('A2', lang('image'));
        $sheet->setCellValue('C2', lang('name'));
        $sheet->setCellValue('B2', lang('code'));        
        $sheet->setCellValue('D2', 'Sys');
        $sheet->setCellValue('E2', 'Front');
        $sheet->setCellValue('F2', 'Back');
        
        $sheet->mergeCells('A1:F1');
        $sheet->setCellValue('A1','Count Stock Staff Name:                                                                                             ' . 'Download date:  ' . date('Y_m_d_H_i_s'));
        $sheet->getRowDimension(1)->setRowHeight(25);
        // Populate data from your database query.
        $row = 3;
        $totalquantity =0;
        // Loop through the product codes and add them to the Excel sheet
        foreach ($codesArray as $code) {
            // Remove any leading/trailing whitespace
            $code = trim($code);
            $product = $this->products_model->getProductByCode($code);
            if (!$product) {
                // Product not found for the current code, return an error
                $response = [
                    'op' => 'error',
            ...
            // Add product image (assuming $imagePath contains the image path).
            $imagePath = '/var/www/html/petland/assets/uploads/' . $product->image;
            if (file_exists($imagePath)) {
                //$imagePath = '/var/www/html/petland/assets/uploads/no_image.jpg';

                $imgfile = base64_decode($imagePath);
                ...
                $drawing->setOffsetX(5); // Adjust the X and Y offsets as needed.
                $drawing->setOffsetY(5);
                $drawing->setWidth(80); // Adjust as needed.
        ...
        // Set the footer with page numbers and total row count.
        $footer = $sheet->getHeaderFooter();
        $footer->setOddFooter('&LPage &P of &N&RTotal Rows: &A');
        $footer->setEvenFooter('&LPage &P of &N&RTotal Rows: &A');

        // Set up the page settings (optional, but you might want to adjust them).
        $pageSetup = $sheet->getPageSetup();
        $pageSetup->setOrientation(PageSetup::ORIENTATION_PORTRAIT); // Portrait orientation.
        $pageSetup->setPaperSize(PageSetup::PAPERSIZE_A4); // Paper size (A4 in this example).

        // Count the total number of rows in the sheet.
        $totalRowCount = $sheet->getHighestRow()-2;

        // You can set the total row count in the footer when you know the count.
        $footer->setOddFooter('&LPage &P of &N&RTotal Rows: ' . $totalRowCount);
        $footer->setEvenFooter('&LPage &P of &N&RTotal Rows: ' . $totalRowCount);
        ...
        // Save the XLSX data to a variable
        $xlsData = null;
        ob_start();
        ...
        // Encode the XLSX data to base64
        $base64Xlsx = base64_encode($xlsData);

        // Create a JSON response
        $response = [
            'op' => 'ok',
            'file' => "data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64," . $base64Xlsx,
        ];
    ...
    public function exportStockCount3($product_code=null) {
        $wh = 1 ;//only HQ
        ...
        $filePath = FCPATH . 'assets/stock_count_' . date('Y_m_d_H_i_s') . '.xlsx';
        // Create a writer for the spreadsheet
        $writer = new PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
        // Save the XLSX data to a variable
    ...
    public function exportStockCount() {
    // Load necessary libraries and models (if needed).

        // Query data from the database.
        $data = $this->db->get('sma_product_both')->result_array();
        ...
        $sheet->setCellValue('D1', 'Expected');
        $sheet->setCellValue('E1', 'Count');

        // Loop through data and populate the spreadsheet.
        $row = 2;
        foreach ($data as $product) {
            ...
            if (!file_exists($imagePath)) {
                ...
                log_message("info","file  found :$imagePath");
                $imageCell = 'A' . $row;
                $drawing = new \PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
                $drawing->setName('Product Image');
                $drawing->setDescription('Product Image');
                ...
                $drawing->setWidth(70); // Adjust as need
                $drawing->setOffsetX(5); // Adjust the X and Y offsets as needed.
                $drawing->setOffsetY(5);
                // Add other product data.
                $sheet->getCell('B' . $row)->setValue($product['code']);
                $sheet->getCell('C' . $row)->setValue($product['name']);
                $sheet->getCell('D' . $row)->setValue($product['quantity_1']);
                $sheet->getColumnDimension('A')->setWidth(12);
                $sheet->getColumnDimension('B')->setWidth(20);
                $sheet->getColumnDimension('C')->setWidth(45);
                $sheet->getColumnDimension('D')->setWidth(8.6);
                $sheet->getColumnDimension('E')->setWidth(8.6);
                $sheet->getRowDimension($row)->setRowHeight(70);

                 // Apply border styles to each cell individually.
                 $highestColumn = $sheet->getHighestColumn();
        $borderRange = 'A' . $row . ':' . $highestColumn . $row;
        $styleArray = [
            'borders' => [
                'outline' => [
                    'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN, // Adjust the border style as needed.
                    'color' => ['argb' => '000000'], // Adjust the border color as needed.
                ],
            ],
        ];
        ...
        $currentDateTime = date('Y-m-d_H-i-s'); // Format it as desired, e.g., '2023-09-02_15-30-45'
        $filename = 'stock_count_' . $currentDateTime . '.xlsx';

        // Set up response headers with the dynamic filename.
    ...
    //     $row = 2;
    //     foreach ($data as $product) {
    //         // Add image (you may need to handle image insertion).
    ...
    //         // $drawing->setHeight(100); // Adjust as needed.
    ...
    //         $sheet->setCellValue('B' . $row, $product['code']);
    //         $sheet->setCellValue('C' . $row, $product['name']);
    //         $sheet->setCellValue('D' . $row, $product['quantity_1']);
    ...
    //     $writer = new Xlsx($spreadsheet);
    //     header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    //     header('Content-Disposition: attachment;filename="stock_count.xlsx"');
    //     header('Cache-Control: max-age=0');
    ...
    // Helper function to embed an image in Excel
    private function imageToExcel($imagePath) {
    ...
    public function getProducts_both($warehouse_id = null)
{
    ...
    // If not Owner/Admin and no $warehouse_id passed, fallback to user warehouse
    if ((!$this->Owner || !$this->Admin) && !$warehouse_id) {
        $user         = $this->site->getUser();
        $warehouse_id = $user->warehouse_id;
    }

    // Prepare action links (unchanged)
    ...
    $delete_link  = "<a href='#' class='tip po' title='<b>" . $this->lang->line('delete_product') . "</b>' data-content=\"<p>"
        ...
        . "<i class=\"fa fa-trash-o\"></i> " . lang('delete_product') . '</a>';
    ...
    // -- Your existing SELECT and FROM for the 'sma_product_both' table --
    $this->datatables
         ->select('productid, image, code, name, quantity_1, quantity_2, (quantity_1+quantity_2) as total_qty, cname, cost, price, rack, alert_quantity')
         ->from('sma_product_both');

    // --------------------------------------------------------------------
    // 1) CAPTURE THE GLOBAL SEARCH VALUE (the user’s input in the main search box)
    // --------------------------------------------------------------------
    $searchValue = $this->input->post('sSearch'); // DataTables typically sends this as ['value' => '...']
    if (!empty($searchValue)) {
        ...
        foreach ($words as $word) {
            ...
            // 4) CONCAT code, name, cname into one field and search it
            $this->datatables->like(
                "CONCAT(code, ' ', name, ' ', cname)", // single "column"
                $word
            );
        }
    }
    ...
    // Hide cost/price columns if not Owner/Admin and not in session->userdata('show_cost/price')
    if (!$this->Owner && !$this->Admin) {
    ...
    // Output the JSON for DataTables
    echo $this->datatables->generate();
}
    ...
    //        // ->select('*')
    //         ->select('productid, image, code, name,quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty, cname, cost, price,  rack, alert_quantity')
    //             ->from('sma_product_both',false);
    //         //$this->datatables->unset_column('unit');
    //         // $this->datatables
    ...
    public function getProducts_transfer_12($warehouse_id = null)
    {
        $this->sma->checkPermissions('index', true);
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : null;
        ...
        if ($warehouse_id || !$warehouse_id) {
           ...
           // ->select('*')
           // ->select('productid, image, code, name,quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty, cname, cost, price,  rack, alert_quantity')
            ->select('productid, image, code, name, transfer_to_branch_1,quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty,  cname')
                ->from('pawfect.sma_products_transfer_suggestion',false)
                ->where('transfer_to_branch_1 !=', 0);
     //  productid, image, code, name, quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty, transfer_to_branch_1,  cname, cost, price
    ...
    public function getProducts_transfer_21($warehouse_id = null)
    {
        $this->sma->checkPermissions('index', true);
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : null;
        ...
        if ($warehouse_id || !$warehouse_id) {
           ...
           // ->select('*')
           // ->select('productid, image, code, name,quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty, cname, cost, price,  rack, alert_quantity')
            ->select('productid, image, code, name,transfer_to_branch_2, quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty,  cname')
                ->from('pawfect.sma_products_transfer_suggestion',false)->where('transfer_to_branch_2 !=', 0);
     //  productid, image, code, name, quantity_1, quantity_2,(quantity_1+quantity_2) as total_qty, transfer_to_branch_1,  cname, cost, price
...
public function index_both($warehouse_id = null)
    {
        $this->sma->checkPermissions("index","products");
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('products')]];
        $meta = ['page_title' => lang('products'), 'bc' => $bc];
        $this->page_construct('products/index_both', $meta, $this->data);
    }
    public function transfer_suggestion_12($warehouse_id = null)
    {
        $this->sma->checkPermissions("index","products");
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('products')]];
        $meta = ['page_title' => lang('products'), 'bc' => $bc];
        $this->page_construct('products/transfer_suggestion_12', $meta, $this->data);
    }

    public function transfer_suggestion_21($warehouse_id = null)
    {
        $this->sma->checkPermissions("index","products");
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('products')]];
        $meta = ['page_title' => lang('products'), 'bc' => $bc];
        $this->page_construct('products/transfer_suggestion_21', $meta, $this->data);
    }
    ...
    public function product_actions($wh = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            if (!empty($_POST['val'])) {
                ...
                elseif ('export_stockcount' == $this->input->post('form_action')) {
                    ...
                    $totalquantity =0 ;
                    foreach ($_POST['val'] as $id) {
                        // Fetch product data and other related information.
                        // ...
                        $product = $this->products_model->getProductDetail($id);                    
                        $quantity = $product->quantity;
                        if ($wh) {
                    ...
                    // Set up the Excel writer and response headers.
                    $filename = 'stockCount ' . date('Y_m_d_H_i_s') . '.xlsx';
                    $writer = new Xlsx($spreadsheet);
                    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                    header('Content-Disposition: attachment; filename="' . $filename . '"');
                    header('Cache-Control: max-age=0');
