...
class Sales_model extends CI_Model
{
    ...
    public function addSale($data = [], $items = [], $payment = [], $si_return = [], $attachments = [])
    {
        ...
        if ($this->db->trans_status() === false) {
            ...
            // $this->db->from('petland.sma_product_both');
            // $this->db->where_in('code', $codes);
            //  // $this->db->group_by('cname'); // Grouping by category name
            // $query = $this->db->get()->result_array();
            // $outstock_msg = "Out of Stock:\n";
            // foreach ($query as $item)
            // {
            //     $q_petland  = (int)$item['quantity_1'];
            //     $q_pawfect = (int)$item['quantity_2'];
            //     $code = $item['code'];
            //     $name = $item['name'];
            //     $total = $q_petland + $q_pawfect;
                
            //     if($total==0 || 1)
            //     {
            //         $outstock_msg = $outstock_msg . $code . "\n" . $name . "\n" . date('Y-m-d H:i:s') . "\n";
            ...
            // }
            // send_to_telegram_group($outstock_msg);  
            return $sale_id;
        }
    ...
    public function getGrandTotal($sale_id)
    {
        $this->db->select("grand_total");
        $q = $this->db->get_where('sales', ['id' => $sale_id]);
        return $q->row()->grand_total;
    }
