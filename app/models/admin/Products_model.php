...
class Products_model extends CI_Model
{
    ...
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, ' . $this->db->dbprefix('products') . '.name as name, ' . $this->db->dbprefix('products') . '.cost')
            ->where("type != 'combo' AND "
                . '(' . $this->db->dbprefix('products') . ".name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR
                concat(" . $this->db->dbprefix('products') . ".name, ' (', code, ')') LIKE '%" . $term . "%')")
            ->limit($limit);
    ...
    public function getProductsByxCategoryAndQuantityLimit($id)
    {
        $q = $this->db->get_where('products', ['category_id' => $id], 1);
        if ($q->num_rows() > 0) {
            return true;
    ...
    public function getProductsByCategoryAndQuantityLimit($limit) {
        // Assuming 'ksi_test2.sma_product_both' is your table name
        $this->db->select('code, name');
        $this->db->from('sma_product_both');
        $this->db->where('(quantity_1 + quantity_2) <', $limit);
       // $this->db->group_by('cname'); // Grouping by category name
        $query = $this->db->get();
        return $query->result_array();
    }
    public function getProductsQuantity($codes) {
        // Assuming 'ksi_test2.sma_product_both' is your table name
        $this->db->select('code, name, quantity_1,quantity_2');
        $this->db->where('(quantity_1 + quantity_2) <', $limit);
       // $this->db->group_by('cname'); // Grouping by category name
        $query = $this->db->get();
        return $query->result_array();
